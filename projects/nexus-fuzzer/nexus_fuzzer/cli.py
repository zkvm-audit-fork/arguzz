...
from nexus_fuzzer.fuzzer import CircuitChecker, CircuitFuzzer, create_circuit_config
...
class NexusFuzzerClient(FuzzerClient):
    ...
    def generate(self):
        assert self.out_dir, "no output directory"
        assert self.zkvm_dir, "no zkvm library"
        circuit_config = create_circuit_config(self.commit_or_branch, self.no_inline_assembly)
