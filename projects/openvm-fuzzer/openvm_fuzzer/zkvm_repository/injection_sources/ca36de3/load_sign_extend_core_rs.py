def load_sign_extend_core_rs() -> str:
    return """use std::{
    array,
    ...
    I::Writes: From<[[F; NUM_CELLS]; 1]>,
{
    ...
    fn execute_instruction(
        ...
        let mut /*INJECTION*/ local_opcode = Rv32LoadStoreOpcode::from_usize(
            instruction
                .opcode
        ...
        let mut /*INJECTION*/ most_sig_limb = match local_opcode {
            LOADB => write_data[0],
        ...
        if fuzzer_utils::is_injection_at_step("LOAD_SIGN_EXTEND_MSL_FLIPPED") {
            let flipped_most_sig_limb = most_sig_limb ^ (1 << (LIMB_BITS - 1));
            fuzzer_utils::print_injection_info(
                "LOAD_SIGN_EXTEND_MSL_FLIPPED",
                &format!("{} => {}", most_sig_limb, flipped_most_sig_limb),
            );
            most_sig_limb = flipped_most_sig_limb;
        }

        // <------------------------ END OF FAULT INJECTION ------------------------>

        let mut /*INJECTION*/ most_sig_bit = most_sig_limb & (1 << (LIMB_BITS - 1));
        ...
        if fuzzer_utils::is_injection_at_step("LOAD_SIGN_EXTEND_MSB_FLIPPED") {
            let flipped_most_sig_bit = most_sig_bit ^ (1 << (LIMB_BITS - 1));
            fuzzer_utils::print_injection_info(
                "LOAD_SIGN_EXTEND_MSB_FLIPPED",
                &format!("{} => {}", most_sig_bit, flipped_most_sig_bit),
            );
            most_sig_bit = flipped_most_sig_bit;
        }
...
>(
    ...
    match (opcode, shift) {
        (LOADH, 0) | (LOADH, 2) => {
            let mut ext = read_data[NUM_CELLS / 2 - 1 + shift].as_canonical_u32();

            // <----------------------- START OF FAULT INJECTION ----------------------->

            if fuzzer_utils::is_injection_at_step("LOAD_SIGN_EXTEND_MSB_FLIPPED") {
                ext = ext ^ (1 << (LIMB_BITS - 1));
            }
        ...
        (LOADB, 0) | (LOADB, 1) | (LOADB, 2) | (LOADB, 3) => {
            let mut ext = read_data[shift].as_canonical_u32();
