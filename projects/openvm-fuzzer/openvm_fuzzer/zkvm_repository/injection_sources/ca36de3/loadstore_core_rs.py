def loadstore_core_rs() -> str:
    return """use std::borrow::{Borrow, BorrowMut};
    ...
    fn execute_instruction(
        ...
        if fuzzer_utils::is_injection_at_step("LOADSTORE_OPCODE_MOD") {
            let allowed_opcode = vec![
                    ...
                    STOREH,
                    STOREB,
                ].into_iter().filter(|x| *x != local_opcode).collect::<Vec<Rv32LoadStoreOpcode>>();
            let new_local_opcode = fuzzer_utils::random_from_choices(allowed_opcode);
            fuzzer_utils::print_injection_info(
                "LOADSTORE_OPCODE_MOD",
                &format!("{:?} => {:?}", local_opcode, new_local_opcode),
            );
            local_opcode = new_local_opcode;
        }
        ...
        let mut /*INJECTION*/ write_data = run_write_data(local_opcode, read_data, prev_data, shift);
        ...
        let mut /*INJECTION*/ output = AdapterRuntimeContext::without_pc([write_data]);


        // <----------------------- START OF FAULT INJECTION ----------------------->

        if fuzzer_utils::is_injection_at_step("LOADSTORE_PC_MOD") {
            let new_pc = _from_pc + 8;
            fuzzer_utils::print_injection_info(
                "LOADSTORE_PC_MOD",
                &format!("{:?} => {:?}", _from_pc, new_pc),
            );
            output = AdapterRuntimeContext {
                to_pc: Some(new_pc),
    ...
    fn generate_trace_row(&self, row_slice: &mut [F], record: Self::Record) {
        ...
        *flags = [F::ZERO; 4];
        match (opcode, record.shift) {
            ...
            (LOADW, 1) => flags[0] = F::ONE, /*INJECTION*/
            (LOADW, 2) => flags[1] = F::ONE, /*INJECTION*/
            (LOADW, 3) => flags[2] = F::ONE, /*INJECTION*/

            (LOADHU, 0) => flags[1] = F::TWO,
            (LOADHU, 1) => flags[0] = F::ONE, /*INJECTION*/
            (LOADHU, 2) => flags[2] = F::TWO,
            (LOADHU, 3) => flags[2] = F::ONE, /*INJECTION*/
            ...
            (STOREW, 1) => (flags[1], flags[2]) = (F::ONE, F::ONE), /*INJECTION*/
            (STOREW, 2) => (flags[1], flags[3]) = (F::ONE, F::ONE), /*INJECTION*/
            (STOREW, 3) => (flags[2], flags[3]) = (F::ONE, F::ONE), /*INJECTION*/

            (STOREH, 0) => (flags[0], flags[1]) = (F::ONE, F::ONE),
            (STOREH, 1) => (flags[1], flags[2]) = (F::ONE, F::ONE), /*INJECTION*/
            (STOREH, 2) => (flags[0], flags[2]) = (F::ONE, F::ONE),
            (STOREH, 3) => (flags[2], flags[3]) = (F::ONE, F::ONE), /*INJECTION*/
